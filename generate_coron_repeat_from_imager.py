...
if __name__ == "__main__":
    ...
    for cfilter in filter_pairs.keys():
        ...
        const1 = ntab1[f"fit_exp_const_{ifilter[0]}"][0]
        slope1 = ntab1[f"fit_linear_lossperyear_{ifilter[0]}"][0]
        startday1 = ntab1[f"fit_startday_{ifilter[0]}"][0]

        amp2 = ntab2[f"fit_exp_amp_{ifilter[1]}"][0]
        tau2 = ntab2[f"fit_exp_tau_{ifilter[1]}"][0]
        const2 = ntab2[f"fit_exp_const_{ifilter[1]}"][0]
        slope2 = ntab2[f"fit_linear_lossperyear_{ifilter[1]}"][0]
        startday2 = ntab2[f"fit_startday_{ifilter[1]}"][0]

        weight = (cwave - iwave[0])/(iwave[1] - iwave[0])
        
        amp = amp1 * (1 - weight) + amp2 * weight
        tau = tau1 * (1 - weight) + tau2 * weight
        const = const1 * (1 - weight) + const2 * weight
        slope = slope1 * (1 - weight) + slope2 * weight
        startday = startday1 * (1 - weight) + startday2 * weight
        print(cfilter, slope, const, amp, tau, startday)

        otab = QTable()
        otab[f"fit_exp_amp_{cfilter}"] = [amp]
        otab[f"fit_exp_tau_{cfilter}"] = [tau]
        otab[f"fit_exp_const_{cfilter}"] = [const]
        otab[f"fit_linear_lossperyear_{cfilter}"] = [slope]
        otab[f"fit_startday_{cfilter}"] = [startday]
        if cfilter == "FND":
            bstr = ""
        else:
            bstr = "_bkgsub"
